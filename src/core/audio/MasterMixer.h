...
//
//  3. Barrier membership is IMPLICIT, not an explicit accept[] gate. A
//     slice joins on its first block and is demoted after it misses
//     kStallTolerance consecutive drains. Thetis can hold an explicit
//     gate because all of its call sites live in one C module; ours are
//     spread across RadioModel, AudioEngine and the slice lifecycle,
//     where a single missed release would wedge ALL audio. Implicit
//     membership self-heals instead, and the gain ramp keeps the
//     rejoin inaudible.
//
...
private:
    ...
    // Consecutive empty drains before a member is demoted and the mix
    // proceeds without it. Two blocks is ~2.7 ms of tolerance.
    static constexpr int kStallTolerance = 2;
    ...
    struct SliceState {
        ...
        bool producing{false};        // barrier member
        int  stalls{0};               // consecutive empty drains
