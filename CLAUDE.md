...
### Current Phase: v0.2.3 shipped 2026-04-24 — RX epic + Alex antennas complete. **3M-1 SSB TX shipped via PR #152 (2026-04-29).** **Phase 3Q chrome layer in PR review on `feature/phase3q-connection-workflow-refactor` 2026-04-30** — status-bar redesign (RxDashboard with BadgePair drop ladder + StationBlock + AdcOverloadBadge + OverflowChip), SVG icon system on StatusBadge (10 icons replacing Unicode glyph prefixes), TitleBar ConnectionSegment with min-filtered RTT, ship-default calibration shifted -12 dB to match residential HF noise floor, PA voltage formula corrected per Thetis convertToVolts (-12 dB sign error fixed; live ANAN-G2 reads 13.3 V), PSU widget dropped after Thetis source-first audit (computeHermesDCVoltage is dead code in Thetis — never displayed), Mbps-glyph byte-escape sweep, Network Diagnostics TX/RX rate fix. Subsequent PRs on the same branch land the unicast-probe / ConnectionPanel rebuild / Add-Radio dialog / spectrum disconnect overlay.  **3M-3a-i (TX EQ + Leveler + ALC) Complete (pending bench) on `feature/phase3m-3-tx-processing` 2026-04-29 — 13 GPG-signed commits across 6 batches; 246/246 tests green; awaiting JJ bench on ANAN-G2.** Next sub-PR is **3M-3a-ii (CFC + CESSB + Pre-emphasis)** on the same epic; 3M-2 CW TX deferred until after the full 3M-3 epic ships AND the HL2 ATT/filter safety audit closes (HL2 TX bench was deferred during 3M-1c after JJ flagged ATT/filter code paths that could damage the radio if engaged — see PR #152 rows 58-60).
