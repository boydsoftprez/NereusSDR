...
Current version: **0.3.1** (set in `CMakeLists.txt`; tagged pre-releases use `vX.Y.Z-rcN` suffix).
...
### Current Phase: v0.3.1 shipped 2026-05-03. Five major chunks landed between v0.2.3 (2026-04-24) and v0.3.1: **3M-1 SSB TX** (PR #152, 2026-04-29); **Phase 3Q connection workflow refactor + chrome layer** (PR #158, 2026-04-30) including single ConnectionState state machine, unicast probe across Layer-3 VPNs, Add Radio dialog with 16-SKU model picker, RxDashboard with BadgePair / StationBlock / AdcOverloadBadge / OverflowChip, SVG icon system, CPU System/App toggle, ship-default cal shifted -12 dB, PA voltage formula corrected per Thetis `convertToVolts`, PSU widget dropped after Thetis source-first audit; **3M-3a-i** (TX EQ + Leveler + ALC) Complete pending bench, 13 GPG-signed commits, 246 tests; **3M-3a-ii** (CFC + CPDR + CESSB + Phase Rotator) Complete pending bench via PR #156, 17 commits, 253 tests, MicProfileManager bundle 91 keys, 19 of 21 factory profiles ported verbatim from `database.cs:9282-9418 [v2.10.3.13]`; **ParametricEq widget port** Complete via PR #159 (10 batches), `src/gui/widgets/ParametricEqWidget.{h,cpp}` drives both TxCfcDialog and TxEqDialog with drag handles, per-band Q sliders, freq sliders, live curve overlay; and **Plan 4 / ui-polish-foundation epic** Complete via PR #166 (~80 commits 2026-05-02 to 2026-05-03) covering TX filter wiring, TX/RX filter overlay on spectrum, filter-preset store + edit dialog, PA cal IA reshape Phases 2-7, PaCalProfile per-board interpolation, swrProtectFactor, MOX-gated passband colors, plus shutdown/persistence/UI fixes. Pre-emphasis remains de-scoped from 3M-3a-ii to 3M-3b (FM-mode work) per master design §7.2. **HL2 ATT/filter safety audit closed in v0.3.1** so HL2 bench is no longer a gating concern (was the deferral cause behind 3M-1c). **Next on this epic: 3M-3a-iii (DEXP/VOX + AMSQ).** Full Thetis-faithful port: 14 DEXP setters per `cmaster.cs:160-206 [v2.10.3.13]` (3 already shipped in 3M-1b: SetDEXPRunVox, SetDEXPAttackThreshold, SetDEXPHoldTime; 11 to add) plus 3 AMSQ setters (`SetTXAAMSQRun/MutedGain/Threshold`). UI: TxApplet `[VOX]` upgrades to DEXP-aware + new `[DEXP]` toggle, Setup → Transmit → DEXP/VOX page (3 group boxes per Thetis IA: grpDEXPVOX + grpDEXPLookAhead + AMSQ NereusSDR-spin section), MicProfileManager bundle gains ~14 keys. 3M-2 CW TX still deferred until after the full 3M-3 epic ships.
