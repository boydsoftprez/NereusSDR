...
Current version: **0.5.1** (set in `CMakeLists.txt`; tagged pre-releases use `vX.Y.Z-rcN` suffix). 3M-2 CW TX is the next major epic; the exact version of the next release is picked by the `/release` skill at release time.
