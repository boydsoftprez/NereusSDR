...
| # | Decision | Options |
| --- | --- | --- |
| D-a | `NotchModel::globalEnabled` default | `true` (AetherSDR `TnfModel.h:52`) or `false` (Thetis `chkTNF` ships unchecked; WDSP `create_notchdb` master run is `0`, `RXA.c:87`). One decision, three files. |
| D-b | `NotchModel::autoIncrease` default | Must be `true` to match `RXA.c:105`, but confirm against Thetis `chkMNFAutoIncrease` before Task 9 ships. |
| D-c | `NotchModel` attribution scope | Task 3 wants a `setup.designer.cs:44167-44179` cite for the visual-notch default, which under the multi-file rule obliges a **fourth** verbatim header. Widen to four files, or move that cite to `DspSetupPages.cpp` (already `setup.cs`-registered). |
| D-d | `adminBusy` semantics | Design §9 has the MNF page hold it while editing and §5.4 has mutators reject writes while it is held, so the page cannot commit its own edit. Task 9 resolved it source-first (Thetis clears `AddActive`/`EditActive` before writing, `setup.cs:17941-17958`), making the flag protect only the panadapter path. Confirm, or drop the flag. |
| D-e | Right-click on empty pan | Design §7 lists "Add notch at X MHz" there. Task 7 dropped it: our empty-pan right-click opens `SpectrumOverlayMenu`, a bespoke popup rather than a `QMenu`, so adding the item changes a shipped gesture. Ctrl + right-click already covers the capability. Restore (converting that popup to a `QMenu`) or accept the drop. |
| D-f | TNF toggle accelerator | Task 8 picked **Ctrl+Shift+N**, verified unclaimed. Spec names no chord. |
