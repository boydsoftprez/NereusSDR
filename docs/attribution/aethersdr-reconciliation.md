...
| `src/gui/SpectrumWidget.h` (Task 11 extension) | `src/gui/SpectrumWidget.cpp:17223-17335` [@1872028c] | Task 11 (3D dBm scale). Adds three private method declarations beside the existing `drawDbmScale` declaration -- `drawDbmScaleChrome(QPainter&, const QRect&)`, `drawDbmScaleLabels(QPainter&, const QRect&, float topDbm, float rangeDb)`, `drawDbmScale3D(QPainter&, const QRect&, float floorDbm)` -- and two public test-seam forwarders beside the existing 3DSS test-seam block, `drawDbmScale3DForTest` and `drawDbmScaleLabelsForTest`, following the `drawSpotMarkersForTest`/`drawNotchMarkersForTest` one-line-forward convention already established in this file. NereusSDR divergence, precisely stated: upstream shares `drawDbmScaleChrome`/`drawDbmScaleLabels` between its OWN `drawDbmScale()` (2D) and `drawDbmScale3D()`. NereusSDR's `drawDbmScale` (2D) is an earlier, independent port from a DIFFERENT upstream state (`SpectrumWidget.cpp:4124` [@0cd4559], predating AetherSDR's own chrome/labels refactor) that this task's own constraints explicitly forbid touching -- "DO NOT REMOVE OR RESTRUCTURE CODE YOU DID NOT ADD ... a 2D user must see zero behavioural change." `drawDbmScaleChrome`/`drawDbmScaleLabels` are therefore ported here as NEW, freestanding private methods with no caller anywhere in the existing 2D path; their only caller is the new `drawDbmScale3D`. `drawDbmScaleLabelsForTest` is beyond the task brief's own listed test seam (which names only `drawDbmScale3DForTest`): added because `dssSpanDb()` floors at `1.0f` (Task 9's `SpectrumWidget::dssSpanDb()`), so `drawDbmScale3D`'s internally-derived `span` can never be non-positive in production, making the ported `rangeDb<=0` guard unreachable through `drawDbmScale3DForTest()` alone. This seam drives the guard directly so the "zero or negative span must not crash or divide by zero" requirement is proven by a real mutation test rather than argued -- see the `.cpp` row and `tests/tst_dss_dbm_scale.cpp::drawDbmScaleLabels_guardsNonPositiveSpan`. **Fast-follow (post-review)**: adds `float dssRoundedSpanDb() const` beside `dssSpanDb()` -- `dssSpanDb()` rounded to the nearest 0.5 dB, factoring a formula upstream inlines independently at each of its three consumers' own call sites (see the `.cpp` row's `drawDbmScale3D` note) into one NereusSDR-original named accessor, after review caught `writeDssMeshUbo` (Task 9) still writing the raw, unrounded value while `drawDbmScale3D` and `buildDssImage` (Task 10) both rounded -- see the `.cpp` row for the concrete disagreement this caused. Also adds `drawDbmScaleChromeForTest`, a test-only forwarder (matching the established `*ForTest()` one-line-forward convention) letting a test reconstruct `drawDbmScale3D`'s exact output from public pieces alone. | "`drawDbmScaleChrome`/`drawDbmScaleLabels`/`drawDbmScale3D` declared, ported from AetherSDR `SpectrumWidget.cpp:17223-17335` [@1872028c]. The existing `drawDbmScale` (2D, ported earlier from `[@0cd4559]`) is untouched; the two new chrome/labels helpers are freestanding additions with no 2D caller, matching this task's constraint against restructuring code it did not add. `drawDbmScaleLabelsForTest` is a NereusSDR-original test seam beyond the brief, needed because `dssSpanDb()`'s existing floor makes the ported degenerate-span guard otherwise unreachable from the brief's own given entry point. Fast-follow adds `dssRoundedSpanDb()`, factoring the rounding formula writeDssMeshUbo, buildDssImage, and drawDbmScale3D must all agree on into one named accessor, after review caught writeDssMeshUbo writing the unrounded value." |
| `src/gui/SpectrumWidget.cpp` (Task 11 extension) | `src/gui/SpectrumWidget.cpp:17223-17335, 13983-13989, 15122-15128` [@1872028c] | Task 11. **`drawDbmScaleChrome`** (`:17223-17261` [@1872028c]): background fill, left border line, up/down arrows -- ported using the SAME literal `QColor` values NereusSDR's existing 2D `drawDbmScale` already uses for this exact chrome (background `QColor(0x0a,0x0a,0x18,220)`, border `QColor(0x30,0x40,0x50)`, arrows `QColor(0x60,0x80,0xa0)`), since upstream's `AetherSDR::ThemeManager` token system (`color.background.0`, `color.text.secondary`, `color.background.2`, ...) has no NereusSDR counterpart -- NereusSDR styles via `StyleConstants.h`, not a runtime theme registry, and the 2D `drawDbmScale`'s own already-shipped literal-colour mapping for this identical visual element is the established, non-guessing translation to reuse rather than inventing a new one. Reuses the existing, already-tested `NereusSDR::DbmStrip::stripRect()` helper (`dbm_strip_math.h`/`.cpp`, covered by `tests/tst_spectrum_dbm_strip.cpp`) for the strip geometry instead of re-inlining `stripX`/`strip` construction a second time in this file, matching the 2D `drawDbmScale`'s own established call pattern at its call site. **`drawDbmScaleLabels`** (`:17263-17312` [@1872028c]): ported with `topDbm`/`rangeDb` as explicit parameters, matching upstream exactly -- unlike the 2D `drawDbmScale`, which reads `m_refLevel`/`m_dynamicRange` members directly via the separate `dbmToY()` helper (a different, NereusSDR-specific mapping this task does not touch or reuse: `dbmToY()` is tied to member state, not parameterizable, and additionally reserves `bandPlanStripHeight()` at the bottom of its content area, a NereusSDR-only refinement absent from upstream's own `drawDbmScaleLabels` that this port does not add, since replicating an unrequested behaviour upstream itself doesn't have would be guessing at scope). The adaptive step ladder (`rawStep=rangeDb/5; >=20?20:>=10?10:>=5?5:2`) calls the existing, already-tested `NereusSDR::DbmStrip::adaptiveStepDb()` instead of re-inlining the identical if/else ladder a third time in this file (the 2D `drawDbmScale` already has it inline; `dbm_strip_math.cpp` already has it extracted and pinned by 8 parametrized test rows in `tst_spectrum_dbm_strip.cpp`) -- byte-identical results, zero new float-boundary risk, and the upstream comment ("Use adaptive step: aim for ~4-6 labels") is preserved directly above the call site. Text drawn via plain `p.drawText()` matching upstream exactly, NOT via the 2D scale's `m_dbmLabelCache`/`QStaticText` shaping cache (a separate, NereusSDR-specific perf addition from an earlier, unrelated task) -- kept out of scope here as the simpler, more directly faithful translation. Braces added to the single-line `if (y < labelTop || y > specRect.bottom() - 5) { continue; }` per this project's C++ style guide (both upstream and the existing 2D `drawDbmScale` omit them on this exact line -- a pure syntactic, zero-behaviour-change translation choice for new code, not a logic change, and not a fix applied to the pre-existing 2D line). The `rangeDb <= 0.0f` guard is ported verbatim; unreachable through `drawDbmScale3D`'s only production call shape today (`dssSpanDb()` floors at `1.0f`) but kept as faithful, upstream-matching defense-in-depth, and proven load-bearing by direct mutation via the new `drawDbmScaleLabelsForTest` seam -- guard removed: `drawDbmScaleLabels_guardsNonPositiveSpan` goes red (`'!anyInk' returned FALSE`, ink drawn where none should be); restored: green again; see `tests/tst_dss_dbm_scale.cpp` and `task-11-report.md`. **`drawDbmScale3D`** (`:17320-17335` [@1872028c]): ported verbatim -- calls `drawDbmScaleChrome` unconditionally (chrome always renders regardless of span, matching upstream's own placement of the guard inside `drawDbmScaleLabels` rather than before the chrome call), rounds `dssSpanDb()` to the nearest 0.5 dB (matching the mesh/CPU-surface rounding upstream's own comment cites, issue "#3937", so labels and the 3D surface agree to the pixel), then calls `drawDbmScaleLabels(p, specRect, floorDbm+span, span)`. `floorDbm` stays an explicit parameter, matching upstream and the task's own "Produces" signature, rather than calling `dssFloorDbm()` internally the way Task 10's `buildDssImage()` does; both call sites below pass `dssFloorDbm()` fresh at the call, matching the NereusSDR convention already established by Task 9's `writeDssMeshUbo` (`ubo[i++]=dssFloorDbm();`) and Task 10's `buildDssImage` (`const float floorDbm=dssFloorDbm();`) -- not upstream's own per-frame `dssFrameFloorDbm` cached local, whose reason to exist (upstream's floor source differs by render mode at ITS call sites) does not apply here, the same simplification Task 10's row above already made and justified for `buildDssImage()`. **Call sites**: upstream's TWO call sites carry genuinely different comments, not a copy-paste duplicate, and each was mapped to its NereusSDR structural counterpart by reading the enclosing function name directly rather than assumed -- upstream's `renderGpuFrame()` call site (`:13983-13989` [@1872028c], comment "dBm strip: both render modes use a readable full-height amplitude reference; the 3D surface itself is perspective-foreshortened.") maps to NereusSDR's `renderGpuFrame()` overlay-build block (the GPU path, `#ifdef NEREUS_GPU_SPECTRUM`); upstream's `paintEvent()` call site (`:15122-15128` [@1872028c], comment "dBm strip: 2D draws a linear dBm axis; 3D maps the ticks onto the front (live) trace's ridge band. Same strip chrome and click targets either way.") maps to NereusSDR's `paintEvent()` (the CPU/QPainter-only path, compiled when the GPU path returns early under `-DNEREUS_GPU_SPECTRUM`). Both NereusSDR call sites replace the single unconditional `drawDbmScale(p, QRect(0,0,w,specH))` with `if (m_spectrumRenderMode==Mode3D) { drawDbmScale3D(p, QRect(0,0,w,specH), dssFloorDbm()); } else { drawDbmScale(p, QRect(0,0,w,specH)); }`, where the `else` arm is the byte-identical original call with the byte-identical original argument expression -- confirmed by `git diff`: zero lines removed from `drawDbmScale`'s own function body (signature through closing brace), only its two call sites gained the branch. Pre-existing NereusSDR comments explaining why the FULL-WIDTH rect must be passed (an earlier, unrelated task's concern) are left in place verbatim above the new branch at both sites. **Test coverage** (`tests/tst_dss_dbm_scale.cpp`): the task brief's own given `labels_followTheFloorAnchorIn3D` proves only that `dssFloorDbm()` moves when `m_nfLerpAverage` moves -- already covered exhaustively by Task 9's `tst_dss_floor_and_span.cpp`, and it never calls `drawDbmScale3D` at all, so it cannot catch a bug where the `floorDbm` PARAMETER is ignored or hardcoded inside the function that actually paints it. Kept as a precondition sanity check and strengthened with a new `render3D_shiftsWithMeasuredFloor`, which pixel-diffs two full off-screen renders at two floors already known (from the test above) to differ by more than 19 dB and requires the resulting `QImage`s to differ -- proven load-bearing by mutation: with `floorDbm` temporarily shadowed to a hardcoded `-120.0f` inside `drawDbmScale3D`, exactly this one test goes red (`'canvasA != canvasB' returned FALSE`) while the other four stay green, including the brief's own `labels_followTheFloorAnchorIn3D` -- confirming that test alone would NOT have caught the bug the task's own required verification #1 warned about ("make sure it binds rather than comparing two values that are equal for an unrelated reason"). Reverted; reconfirmed 7/7 (5 test functions + init/cleanup) green. Both brief-given crash-safety tests (`drawDbmScale3D_paintsWithoutCrashing`, `degenerateRect_isRefusedNotDivided`) kept verbatim. Required verification #3 (degenerate span) added as `drawDbmScaleLabels_guardsNonPositiveSpan`, described under the `.h` row and mutation-proven above. **Fast-follow (post-review)**: `writeDssMeshUbo`'s `rangeDb` field (`ubo[i++] = dssSpanDb();`) wrote the RAW, unrounded span while `drawDbmScale3D` and `buildDssImage` (Task 10) both rounded to the nearest 0.5 dB, so `dss_mesh.vert`'s `rangeDb` uniform (front-ridge height mapping) could disagree with the drawn scale and the CPU fallback by up to 0.25 dB whenever the configured dBm range was not already an exact 0.5 dB multiple -- several pixels on a tall strip at a narrow range. This also meant `drawDbmScale3D`'s own ported doc comment ("Round the span the same way the mesh/CPU surface do... buildDssImage / renderGpuFrame use std::round(span*2)/2") was true of upstream but false of this port until fixed. Closed by factoring the formula into `dssRoundedSpanDb()` (new, `.h` row) and routing all three consumers through it, so the drift class is structurally impossible to reintroduce at one call site alone. New test `tests/tst_dss_dbm_scale.cpp::roundedSpanDb_roundsToNearestHalfDb` pins the accessor's own arithmetic (33.25 -> 33.5); `render3D_matchesTheRoundedSpanNotTheRawSpan` pins the actual agreement claim by independently reconstructing `drawDbmScale3D`'s expected output from public pieces (`drawDbmScaleChromeForTest` + `drawDbmScaleLabelsForTest`) fed the SAME `dssRoundedSpanDb()` value and comparing pixel-for-pixel against the real black-box render -- `writeDssMeshUbo` itself cannot be unit-tested (needs live QRhi resources, per Task 9's own precedent), so this is the strongest agreement pin reachable from a unit test; the UBO side was additionally confirmed correct once, empirically, via temporary instrumentation in a real app run (`ubo[2](rangeDb)=77.0000` exactly matched `dssRoundedSpanDb()=77.0000`, differing from the raw `dssSpanDb()=76.9995`), reverted before commit. Mutation-proven: `dssRoundedSpanDb()`'s rounding removed -> `roundedSpanDb_roundsToNearestHalfDb` fails; `drawDbmScale3D` reverted to reading raw `dssSpanDb()` -> `render3D_matchesTheRoundedSpanNotTheRawSpan` fails ALONE (the other 6 test functions stay green, isolating the catch); `writeDssMeshUbo` reverted to the original raw-write bug -> confirmed, honestly, that NO unit test catches it (193/193 gui tests stay green), which is exactly why the empirical runtime check above exists. | "`drawDbmScaleChrome`/`drawDbmScaleLabels`/`drawDbmScale3D` ported from AetherSDR `SpectrumWidget.cpp:17223-17335` [@1872028c]; call sites wired at `renderGpuFrame()` (upstream `:13983-13989`) and `paintEvent()` (upstream `:15122-15128`), each upstream comment preserved at its correctly-mapped NereusSDR counterpart. Colours de-themed to the same literal `QColor` values the existing 2D `drawDbmScale` already uses (no `AetherSDR::ThemeManager` counterpart in NereusSDR); the adaptive-step ladder and strip-rect geometry reuse the existing tested `dbm_strip_math.h` helpers instead of re-inlining. `floorDbm` stays an explicit parameter per upstream and the task's own signature; both call sites pass `dssFloorDbm()` fresh, matching the Task 9/10 convention rather than upstream's per-frame cached local, which has no NereusSDR equivalent reason to exist. The 2D `drawDbmScale` is completely untouched -- confirmed by `git diff` showing zero removed lines inside its function body, only its two call sites gaining a mode branch whose `else` arm is the original call unchanged. Fast-follow: writeDssMeshUbo's rangeDb field wrote the raw, unrounded dssSpanDb() while drawDbmScale3D and buildDssImage both rounded, letting the GPU mesh's height mapping disagree with the drawn scale by up to 0.25 dB; fixed by routing all three through the new dssRoundedSpanDb() accessor, confirmed empirically in a real app run and mutation-proven at the drawDbmScale3D end (the only end a unit test can reach)." |
