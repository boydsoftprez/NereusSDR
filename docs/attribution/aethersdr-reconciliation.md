...
| `src/gui/PanFloatingWindow.h` | `src/gui/PanFloatingWindow.{h,cpp}` [@1e0718ad] | Structural port: top-level `Qt::Window` wrapper that adopts a `PanadapterApplet` for multi-monitor detach. 2026-08-08 expansion (bench: floated pan froze and had no chrome) brings across upstream's `adoptApplet()` / `takeApplet()` reparent contract (never through `setParent(nullptr)`, to avoid the transient NSWindow that corrupts macOS responder chains), main-window parenting for z-order, `saveWindowGeometry()` / `restoreWindowGeometry()` under `FloatingPan_<panId>_Geometry`, and dock-on-close. NereusSDR keeps the native window frame where upstream offers a frameless mode via `FramelessResizer`. | "Top-level pan detach window ported structurally from AetherSDR `src/gui/PanFloatingWindow.{h,cpp}` [@1e0718ad]." |
| `src/gui/PanFloatingWindow.cpp` | `src/gui/PanFloatingWindow.{h,cpp}` [@1e0718ad] | Same as `.h`. | "Same as `.h`." |
